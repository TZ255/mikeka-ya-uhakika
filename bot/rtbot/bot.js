        ...
        for (let t of tksn) {
            ...
            if (process.env.environment === "local") {
                try {
                    ...
                    await bot.api.sendMessage(imp.shemdoe, `${hookPath} set as webhook for ${t.NAME}`)
                } catch (error) {
                    console.error(error?.message)
                    await bot.api.sendMessage(imp.shemdoe, `Failed to set webhook for ${t.NAME}`)
                }
            }
